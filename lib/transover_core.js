...
Core.callAPI = (word, onresponse, sendResponse) => {
  ...
  const options = {
    url: 'https://sapir.artsrn.ualberta.ca/cree-dictionary/click-in-text?q=' + encodeURIComponent(word),
    dataType: 'json',
...
Core.parseAPIResponse = (data, word, sendResponse) => {
  ...
  if (!data.results || data.results.length === 0) {
    ...
    //  The difference is that the user might not be certain what they are looking for on Itwewina, thus the need
